...
class StockController extends Controller
{
    ...
    public function addstock()
    {
        ...
        // Find the last invoice that matches only the middle invoice part
        ...
        if ($lastInvoice) {
            // Extract exact substring that starts with INV-xxxx-xxxx
        ...
        $invoiceId = $prefix . str_pad($nextSeq, 4, '0', STR_PAD_LEFT);

        while (Purchase::where('invoice', 'like', "%{$invoiceId}%")->exists()) {
            $nextSeq++;
            $invoiceId = $prefix . str_pad($nextSeq, '0', 4, STR_PAD_LEFT);
        }


        return view('admin.stock.create', compact('products', 'suppliers', 'colors', 'sizes','warehouses','invoiceId'));
    }

    public function stockStoreOld(Request $request)
    {
    ...
    public function stockStore(Request $request)
    {
        ...
        try {
            ...
            // Generate Invoice
            $invoicePrefix = "STL-{$validated['season']}-{$validated['invoice']}";
